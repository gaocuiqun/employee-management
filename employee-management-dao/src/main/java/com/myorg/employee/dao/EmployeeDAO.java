...
public class EmployeeDAO {
  ...
  public int create(CreateEmployeeCmd c) {
    int rowsAffected = jdbcTemplate.update("UPDATE employeedb.employee SET name = ?, password = ?, desc = ? WHERE id = ?", c.getName(), c.getPassword(), c.getDesc(), c.getId());
    if(rowsAffected > 0) {
      return rowsAffected;
    } else {
      return jdbcTemplate.update("INSERT INTO employeedb.employee(id,name,password,desc) VALUES (?,?,?,?)", c.getId(),c.getName(),c.getPassword(),c.getDesc());
    }
  }

  public EmployeeVo retrieveByRowid(RetrieveByRowidCmd c) {
    return (EmployeeVo) jdbcTemplate.queryForObject("SELECT id, name, password, desc FROM employeedb.employee WHERE rowid = ? ", rowMapper, c.getRowid());
  }

  public EmployeeVo retrieve(RetrieveEmployeeCmd c) {
    return (EmployeeVo) jdbcTemplate.queryForObject("SELECT id, name, password, desc FROM employeedb.employee WHERE id = ? ", rowMapper, c.getId());
  }

  public int update(UpdateEmployeeCmd c) {
    return jdbcTemplate.update("UPDATE employeedb.employee SET name = ?, password = ?, desc = ? WHERE id = ?", c.getName(), c.getPassword(), c.getDesc(), c.getId());
  }
  ...
  public EmployeeListVo query(QueryCommand q) {
    if(q.getPageNumber() > 0
      ...
      String sql = String.format("SELECT id, name, password, desc FROM employeedb.employee %s ", where.toWhereClause(q));
      logger.info(sql);
      return EmployeeListVo.newBuilder()
